...
Run a non-interactive build in-container by overriding the entrypoint with `headless_runner.py` flags:

```powershell
docker compose run --rm `
  --entrypoint bash `
  web `
  -lc "python code/headless_runner.py --commander 'Krenko, Mob Boss' --primary-choice 1 --add-lands true"
```

- Any `headless_runner.py` CLI flag (`--commander`, `--primary-choice`, `--fetch-count`, `--bracket-level`, etc.) or its matching `DECK_*` environment variable can be used; see `headless_runner.py --help` or the env-var reference below.
...
| `config/` | `/app/config` | Bracket policy, card list overrides, partner synergy analytics data. |
