...
- Tagging: cards that let you name a creature type for generic tribal support (e.g. Adaptive Automaton, Arcane Adaptation) are now tagged `Kindred Support`, so they surface for any "{X} Kindred" Deck Builder theme.

### Changed
- Deck Builder: the Protection step now caps how many Equipment and Aura cards can count toward the protection target to 2-3 total (combined), since EDHREC rankings favor Equipment heavily but most of it can't protect a creature the same turn it's cast (it still needs to be equipped/enchanted separately). Other protection spell types are unaffected and still fill the rest of the target; decks wanting more Equipment/Auras should lean into that as a dedicated theme instead.
- Deck Builder: the Protection step's board-relevance filter now also includes cards that have both a board-relevant scope (your permanents/blanket/targeted) and an opponent-facing scope from another mode (e.g. the dual-scope phasing cards below, which can target either board), instead of excluding them entirely just because an opponent-facing scope tag was present.
- Deck Builder: finishing a build now automatically navigates to the finished deck's summary page instead of leaving you on the builder screen.
- Web: the card detail page now has its own search bar (next to the Back button), including the same `tag:`/`theme:` autocomplete dropdown used on the Card Browser and Owned Library, so you can start a new card search without navigating back first.

### Fixed
- Deck Builder: Changeling cards (e.g. Morophon the Boundless, Prideful Feastling), which are every creature type per the actual game rules, previously only counted toward their literal printed type (usually Shapeshifter) for Kindred theme candidate pools, so builds like an "Otter Kindred" deck could undercount available creatures and only ever pick up Changelings as generic last-resort filler in the end-of-build creature floor backfill step, not during the actual theme-based creature addition. The Deck Builder now also pulls in `Changeling`-tagged cards for any "{X} Kindred" theme during creature addition itself (not just the backfill step), the same way it already does for generic tribal-support cards (Adaptive Automaton, Roaming Throne, etc.), without adding every possible Kindred tag to each Changeling card's own tag list. The manual builder's card browser now recognizes Changelings the same way, so they rank and highlight correctly in the Creatures/On-Brand sections instead of sorting to the bottom.
- Tagging: an audit of the tagger's own regex patterns (outside the Scryfall Oracle Tag comparison) found the `Midrange` archetype tag's "enters the battlefield, ..." patterns were dead due to MTG's 2020 Oracle text templating change (now just "enters, ..."), leaving ~625 real cards untagged; also fixed the `Removal` tag's reversed-word-order `sacrifices target` pattern (real text reads "target player sacrifices...") which now correctly catches 107 more cards, corrected `Pillowfort`'s singular "a planeswalker" wording to the real plural "planeswalkers" phrasing, modernized `Landfall`'s trigger text fallback, and removed a batch of harmless dead "-s conjugation" patterns across Aristocrat/Counterspell/Board Wipe/Graveyard Recursion pattern lists that never matched real oracle text. A full pipeline re-tag was run to apply the corrected tags.
- Tagging: cards that phase permanents out (e.g. `Oubliette`, `Teferi's Protection`, `The City on the Edge of Forever`) almost never got scope-aware `Protective Effects`/`Removal`/`Board Wipes` tags, because the phasing scope detector's whole-word check could never match its own "phas" search key. Fixed that bug along with several narrower pattern gaps (sentence-boundary phrasing, "you control"-restricted cards spuriously also getting an opponent tag, missing land/planeswalker noun classes, pronoun self-references, and an anti-phasing card like `Spatial Binding` being misread as causing phasing), and removed the incorrect `Phasing` tag from `Sphinx of the Second Sun` (a false positive from an un-word-boundaried text match). Also corrected `Galadriel's Dismissal`, `Brokers Confluence`, `March of Swirling Mist`, `Slip Out the Back`, and several other cards with bare "target creature/permanent phases out" wording (no "you control"/"opponent" restriction, so the caster could target either board): they no longer get a `Board Wipes` tag where it doesn't apply, and now correctly get both `Protective Effects` and `Removal` since they could go either way. A full pipeline re-tag was run to apply the corrected tags.
- Deck Builder: the Protection step's board-relevance filter only recognized a tag if it literally contained the word "protection", so most protective cards (anything tagged only via the generic `Protective Effects` tag, e.g. Hexproof/Ward/Shroud/Indestructible/Phasing grants) never made it into the candidate pool at all. The filter now recognizes the `Protective Effects` tag directly.
- Deck Builder: the build wizard's "Back" button pointed at a leftover route from an older step-by-step flow; clicking it re-rendered the entire builder page nested inside itself instead of navigating anywhere useful. Removed the button since "Restart Build" and "New build" already cover what it was meant to do.
...
- Manual Deck Builder: pool cards now show a "Sorted by" indicator and a "Why this card?" note (theme matches, EDHREC popularity tier, new-card flag, role fit); a new "Theme Match" sort ranks cards by overlap with your build's selected themes.
- Manual Deck Builder: a new "+ Add Land Package" button pre-adds basic lands (split evenly across your commander's colors, up to your basic-land ideal) plus the standard staple utility lands (Command Tower, Reliquary Tower, etc.).
- Manual Deck Builder: the pool browser has been replaced with a single categorized page: New Cards, On-Brand Cards, Related Synergy (cards matching your commander's other themes you didn't pick), Creatures, Instants, Sorceries, Utility Artifacts, Enchantments, Battles, Planeswalkers, Utility Lands, Mana Artifacts, and Lands, each independently searchable/paginated. A table-of-contents lets you jump straight to any section instead of scrolling (now pinned to the top of the page as you scroll), and a single search box filters across every category at once (the whole pool, not just the visible section).
- Manual Deck Builder: each pool category is capped to a curated top 50 cards (these are recommendations, not an exhaustive catalog), and every section now starts expanded instead of needing to be unfolded.
- Manual Deck Builder: five new pool sections (Ramp, Removal, Card Draw, Board Wipes, Protection) surface cards that fill those roles regardless of theme or card type, so off-brand utility cards are easy to find instead of only showing up when they happen to match your deck's selected themes. A card can appear in one of these and its usual type section (e.g. a removal creature shows in both Removal and Creatures).
- Manual Deck Builder: pool cards now have a "Choose Printing"/foil toggle, same as the rest of the app, so you can preview a specific set/art and price before adding a card.
- Manual Deck Builder: the "Why this card?" panel now lists every one of a card's theme tags, highlighting which ones match your deck's selected theme(s) versus which one just explains its role-bar bucket (e.g. a Discard-Matters card that's also tagged Card Draw now shows both, instead of only the role).
- Manual Deck Builder: basic lands and "any number of cards named X" cards in the current deck panel now have a quantity input to set an exact copy count directly, instead of clicking Add/Remove one at a time.
- Manual Deck Builder: the search box now supports the same Scryfall-style query syntax as the public card browser API - `t:`/`type:` (e.g. `t:creature` matches any card with Creature in its type line), `o:`/`oracle:` (rules text), `c:`/`color:`, `id:`/`identity:`, `m:`/`mana:`, `mv:`/`cmc:`, and `pow:`/`tou:`/`loy:`, each supporting `:`, `=`, `>`, `<`, `>=`, `<=`, `!=`, and negation with a leading `-` - in addition to plain name search.
- Manual Deck Builder: once a card is added to the deck it's hidden from the pool (it reappears if removed), except basic lands and the small set of "any number of cards named X" cards, which always stay visible.
- Manual Deck Builder: the old role/sort dropdown pool filter has been removed in favor of the categorized layout above; the role health bar is unaffected.
- Manual Deck Builder: the role health bar's "Threats" pill has been split into "Protection", "Board Wipe", and "On-Theme" (creatures/planeswalkers matching one of your selected themes), each tracked against its own target count; the health bar now stays pinned to the top of the page alongside the category table-of-contents as you scroll, instead of scrolling out of view.
- Manual Deck Builder: pool cards now show a hover panel (instead of an inline "Why this card?"/"Other Good Options" block) with the card's role, matched themes, and full tag list, keeping the pool grid focused on the cards themselves.
- Manual Deck Builder: the commander's card image now appears above the current deck panel, and both stay pinned to the right side of the page as you scroll through the pool.
- Manual Deck Builder: the pinned category table-of-contents/role bar is now scoped to the pool column instead of spanning the full page, so it no longer overlaps the commander's card image; the Save Deck/Export CSV/Export TXT buttons now stick to the deck panel instead of floating independently as you scroll.
- Manual Deck Builder: adding or removing a card now updates the page in roughly 150-250ms instead of over a second (batched card lookups, cached compliance policy files, and disabled per-render template reload checks); the pool list also no longer visually "jumps" to a different category when a card leaves/re-enters the one you're viewing.
- Manual Deck Builder: the current deck panel now groups added cards by card type (Battles, Planeswalkers, Creatures, Instants, Sorceries, Artifacts, Enchantments, Lands, then Other), matching the order used on the finished deck's Type Summary, instead of grouping by role. Each card also lists the theme(s) and ideal role(s) (Ramp, Removal, Card Draw, Board Wipe, Protection) it's filling, e.g. "Preston, the Vanisher (Rabbit Kindred, Removal)", so you can see at a glance how many "core" cards vs. role-fillers are in the deck.
- Manual Deck Builder: removed the non-functional duplicate "Build Manually" nav link and home-page button (both opened the same New Deck modal with no manual-specific behavior); the "Build Manually" option inside that modal is the only entry point now.
- Manual Deck Builder: fixed the "Lands" category being empty in every build and "Utility Lands" incorrectly including plain mana-fixing lands (e.g. Command Tower, fetch lands) - lands are now split correctly, with only lands that have a genuine extra effect beyond mana-fixing showing up as Utility.
- Manual Deck Builder: starting a new manual build without saving the previous one (or right after using "Edit Deck") no longer carries over the old deck's cards, pool, or save target - each new build now starts fresh.
- Manual Deck Builder: the Ramp/Removal/Card Draw/Board Wipes/Protection pool categories now sort purely by EDHREC popularity like the other type-based categories, instead of favoring theme matches first.
- Manual Deck Builder: the Protection category and role bar no longer count cards whose protective ability only affects the card itself (e.g. a creature with its own Ward or a self-buffed indestructible counter) - only cards with evidence of granting protection to other permanents you control count toward the Protection ideal.
- Manual Deck Builder: the card hover preview panel no longer disappears when hovering over a card's name text in the deck panel list; hovering anywhere on a deck-list row now works.
- Card tagging: added a new `Graveyard Hate` tag for cards that deny opponents the use of their graveyard (e.g. Kaya, Geist Hunter), separate from `Board Wipes`; modal cards that do both (e.g. Farewell, Shadows' Verdict) now get both tags instead of only `Board Wipes`.
- Public API: `POST /api/v1/builds` now accepts `mode: "manual"`, opening a fully interactive build session equivalent to the Manual Deck Builder - `GET /{build_id}/pool` browses the categorized card pool, `POST /{build_id}/manual/add` and `/manual/remove` update the deck-in-progress, `POST /{build_id}/manual/set-count` sets an exact copy count for unlimited-copy cards, `POST /{build_id}/manual/land-package` pre-adds a starting land base, `GET /{build_id}/manual/search` searches the legal card pool, `GET /{build_id}/manual/suggestions` surfaces alternative cards for a given pick, and `POST /{build_id}/manual/save` finalizes the deck so it shows up in `GET /api/v1/decks` like any other build.
- Public API: manual-mode responses (`manual/add`, `manual/remove`, `manual/set-count`, `manual/land-package`) now include a `mana_overview` field (pip distribution, mana sources, mana curve for the deck-in-progress), and a new `GET /{build_id}/manual/deck` endpoint returns the deck panel/role bar/compliance/mana overview bundle without mutating anything, for an initial screen load.

### Changed
- All Cards browser and Owned Library: replaced the separate color/type/rarity dropdowns and CMC/power/toughness range inputs with a single Scryfall-style search box, matching the syntax already used in the Manual Deck Builder and public card API (`t:`/`c:`/`id:`/`tag:`/`pow:`/`tou:`/`mv:`, plus `r:`/`is:new`/`set:` on the All Cards browser). Plain text still does the existing fuzzy/typo-tolerant name search; theme chips and sort options are unchanged. Clicking a card's color pip or color-identity badge now searches by that color instead of using the removed dropdown.

### Fixed
- Card database: refreshed the underlying card list and card-recommendation pipeline; "New Cards" was showing mostly old reprints due to stale data, and a small number of un-set/joke cards (non-creature "Hero" cards and a Plane card) were incorrectly showing up in the pool instead of being filtered out.
- Card tagging: fixed a false-positive "Ward" tag caused by unbounded substring matching (e.g. "toward"/"Warden" were being read as containing "ward"); also improved self-granted-ability detection for legendary cards whose oracle text uses their short name (e.g. "Mondrak" instead of "Mondrak, Glory Dominus").
...
- `POST /api/v1/builds/{id}/remove-card` and `POST /api/v1/builds/{id}/remove-card/undo`: mirrors the web build review's "Remove Card"/Undo for guided-mode API clients (like the mobile app), removing a card from the deck-in-progress and freeing its slot for later stages, with a full undo
- `POST /api/v1/builds/{id}/rerun`: re-runs the most recently completed guided-mode stage to pull a fresh batch of cards without advancing further, mirroring the web build review's "Rerun stage" action
...
- Groundwork for multi-printing card images: a new printings index (`card_files/processed/card_printings.parquet`) tracks every paper printing of every card, and the image cache can now store images per-printing (`card_files/images/{Card Name}/{size}/{scryfall_id}.jpg`) instead of just one image per card name; `IMAGE_CACHE_MODE=default|full` controls whether only the best printing or every printing gets downloaded (default: `default`, same footprint as before). `/api/images/{size}/{card_name}` accepts an optional `printing` (Scryfall ID) query param, and `GET /api/printings/{card_name}` lists a card's known printings; both are additive and don't change existing behavior.
- A "Choose Printing" button on card tiles across the web build review, Finished Decks (list and thumbnail views), Card Browser, Owned Library, and the Card Details page opens a picker of that card's known printings (art/set thumbnails); the selection is session-scoped (like card locks), swaps the tile's artwork immediately, round-trips through permalinks, and is shared across all pages (choose it in one place, it shows up in the others too). The Owned Library's picker is cosmetic only and doesn't affect owned-card matching/filtering. The picker panel opens as a floating popup positioned next to the button, so it's never squished by a narrow card tile.
- Per-printing pricing: the price service now tracks each printing's own price (not just the cheapest printing per name), and the web build review's price overlay fetches the selected printing's price instead of always showing the cheapest printing's price
...
- Fetch lands are no longer auto-added (or suggested as swap alternatives) unless they can actually search for a basic land type in the commander's color identity, preventing off-color fetches like a Plains-fetching land showing up in a mono-red deck
- Tribal ("kindred") lands tied to a specific creature type (e.g. `Seraph Sanctuary`, an Angel land) are no longer added to decks that aren't actually built around that tribe, so they don't take up land slots purely for their fallback colorless mana ability
- Fixed missing `Tokens Matter`/`Token Creation` tags on cards that create a token via a copy effect without ever using the literal phrase "creature token" (e.g. `Hashaton, Scarab's Fist`, `Kiki-Jiki, Mirror Breaker`)
