...
```
CLI entry point: `python code/main.py`. Headless convenience runner: `python code/headless_runner.py --commander "..." --primary-choice 1`.
...
### Guest mode
The app works without any sign-up. Unauthenticated visitors automatically use a shared Guest account; decks are saved per-session but are not private.

### Registration & login
- Register at `/auth/register` (username, email, password).
- Log in at `/auth/login`; a session cookie is set on success.
- Your decks and owned-card lists are isolated in user-specific directories.
...
- Run headless (non-interactive) builds: `python code/headless_runner.py --commander "..." --primary-choice 1 --add-lands true`.
- In Docker, set `APP_MODE=cli` (and optionally `DECK_MODE=headless`) to switch the container entrypoint to the CLI.
- Config precedence is CLI flags > environment variables > defaults.
- Dual-commander support (feature-flagged): `--secondary-commander` or `--background` (mutually exclusive) can be supplied alongside `--enable-partner-mechanics true` or `ENABLE_PARTNER_MECHANICS=1`; Partner With and Doctor/Doctor’s Companion pairings auto-resolve (respecting opt-outs), and dry runs (`--dry-run`) echo the resolved pairing.
...
| `config/` | `/app/config` | Bracket policies, themes, card lists, partner synergy analytics |
...
config/               Bracket policies, themes, card lists
