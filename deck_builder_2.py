...
choice = inquirer.confirm(message='', default=True).execute()
if choice:
    use_scrython = True
...
class DeckBuilder:
    ...
    def questionnaire(self, inq_type, inq_default='', inq_choices=[]):
        if inq_type == 'Text':
            result = inquirer.text(message='', default=inq_default).execute()
            return result
        
        if inq_type == 'Number':
            result = inquirer.text(message='', default=inq_default, validate=NumberValidator(float_allowed=True)).execute()
            return float(result)
        
        if inq_type == 'Confirm':
            result = inquirer.confirm(message='', default=inq_default).execute()
            return result
        
        if inq_type == 'Choice':
            result = inquirer.select(message='', choices=inq_choices).execute()
            return result
    ...
    def determine_ideals(self):
        ...
        if use_scrython:
            ...
            if choice:
                ...
                self.max_card_price = float(answer)
            else:
