...
class DeckBuilder:
    ...
    def determine_commander(self) -> None:
        ...
        """
        logging.info("Starting commander selection process")
        
        try:
            # Load commander data using builder_utils
            df = builder_utils.load_commander_data()
            logging.debug("Commander data loaded successfully")
            
            # Select commander
            commander_name = self._select_commander(df)
            logging.info(f"Commander selected: {commander_name}")
            
            # Confirm selection
            commander_data = self._confirm_commander(df, commander_name)
            logging.info("Commander selection confirmed")
            
            # Initialize commander
            self._initialize_commander(commander_data)
            logging.info("Commander initialization complete")
            
        except DeckBuilderError as e:
            logging.error(f"Commander selection failed: {e}")
            raise
        except Exception as e:
            logging.error(f"Unexpected error in commander selection: {e}")
    ...
    def _select_commander(self, df: pd.DataFrame) -> str:
        ...
        while True:
            try:
                ...
                logging.info("Multiple commander matches found")
                ...
                logging.warning(f"Commander selection attempt failed: {e}")
                continue
    ...
    def _confirm_commander(self, df: pd.DataFrame, commander_name: str) -> Dict:
        ...
        try:
            ...
            logging.error(f"Commander confirmation failed: {e}")
            raise
    ...
    def _initialize_commander(self, commander_data: Dict) -> None:
        ...
        try:
            ...
            logging.debug("Commander attributes initialized successfully")
            
        except Exception as e:
            logging.error(f"Commander initialization failed: {e}")
    ...
    def commander_setup(self) -> None:
        ...
        try:
            ...
            logging.info("Commander setup completed successfully")
            
        except CommanderValidationError as e:
            logging.error(f"Commander validation failed: {e}")
            raise
        except DeckBuilderError as e:
            logging.error(f"Deck building initialization failed: {e}")
            raise
    ...
    def determine_color_identity(self) -> None:
        ...
        try:
            ...
            logging.warning(f"Unknown color identity: {validated_identity}")
            self.color_identity_full = 'Unknown'
            self.files_to_load = ['colorless']
            
        except CommanderColorError as e:
            logging.error(f"Color identity validation failed: {e}")
            raise
        except Exception as e:
    ...
    def read_csv(self, filename: str, converters: dict | None = None) -> pd.DataFrame:
        ...
        try:
            ...
            for i in range(0, len(df), CSV_PROCESSING_BATCH_SIZE):
                ...
                processed_batch = setup_utility.process_card_dataframe(batch, skip_availability_checks=True)
            ...
            logging.debug(f"Successfully read and validated {filename}_cards.csv")
            #print(df.columns)
            return df
    ...
    def _load_and_combine_data(self) -> pd.DataFrame:
        ...
        """
        logging.info("Loading and combining data from CSV files...")
        all_df = []
        ...
        try:
            ...
            logging.error(f"Error loading and combining data: {e}")
            raise
    ...
    def _split_into_specialized_frames(self, df: pd.DataFrame) -> None:
        ...
        try:
            ...
            logging.error(f"Error splitting DataFrames: {e}")
    ...
    def setup_dataframes(self) -> None:
        ...
        try:
            ...
            logging.info("DataFrame setup completed successfully")
            
        except (CSVError, EmptyDataFrameError, DataFrameValidationError) as e:
            logging.error(f"Error in DataFrame setup: {e}")
            raise
